...
parser.add_argument('--final_checkpoint', type=str,
                    default='weights/refinedet_vgg16_24.h5',
...
for cls, mAP_uf, mAP_f in zip(
        VOC_CLASSES[1:], meanAP_unfiltered, meanAP_filtered):
...
print(' {:.2f}% (unfiltered) --'.format(
    100*np.mean(meanAP_unfiltered)), end="")
print(' {:.2f}% (difficult filtered)'.format(
    100*np.mean(meanAP_filtered)))
