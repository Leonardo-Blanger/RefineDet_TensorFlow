...
class PhotometricDistortions:
    def __init__(self):
        ...
        self.random_saturation = \
            RandomSaturation(lower=0.5, upper=1.5, prob=0.5)
    ...
    def __call__(self, image, boxes=None):
        def _f(image, boxes=None):
            ...
            top = tf.random.uniform(shape=(), minval=0,
                                    maxval=new_height - height + 1,
                                    dtype=tf.int32)
            left = tf.random.uniform(shape=(), minval=0,
                                     maxval=new_width - width + 1,
                                     dtype=tf.int32)

            image = tf.image.pad_to_bounding_box(
                image - self.background, top, left,
            ...
            shift = tf.convert_to_tensor([left, top, left, top],
                                         dtype=boxes.dtype)
...
class RandomCrop:
    ...
    def __call__(self, image, boxes=None):
        ...
        def _loop_body(try_cnt, image, boxes=None):
            ...
            crop_height = tf.cast(tf.cast(height, tf.float32) * ratio_height,
                                  tf.int32)
            crop_width = tf.cast(tf.cast(width, tf.float32) * ratio_width,
                                 tf.int32)

            top = tf.random.uniform(shape=(), minval=0,
                                    maxval=height - crop_height + 1,
                                    dtype=tf.int32)
            left = tf.random.uniform(shape=(), minval=0,
                                     maxval=width - crop_width + 1,
                                     dtype=tf.int32)
            ...
            def _adjust_boxes(try_cnt, image, boxes=None):
                ...
                def _resize(image, boxes):
                    ...
                    shift = tf.convert_to_tensor([left, top, left, top],
                                                 dtype=boxes.dtype)
                    boxes -= tf.pad(shift, [[0, tf.shape(boxes)[1] - 4]])
                    boxes = utils.clip_boxes(boxes, 0, 0,
            ...
            ar = tf.cast(crop_height, tf.float32) / tf.cast(crop_width,
                                                            tf.float32)
